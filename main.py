...
FILENAME = f'../AlpacaKernel-r11-{TIMESTAMP.strftime("%Y%m%d-%H%M")}.zip'
SIGNED_FILENAME = f'../AlpacaKernel-r11-{TIMESTAMP.strftime("%Y%m%d-%H%M")}-signed.zip'
COMPILER_STRING = 'GCC 9.x'
KERNEL_VERSION = 'Alpaca, r11, LTO'
DEVICE = 'platina'
...
os.system(f'sed s/Alpaca/Alpaca-{repo.active_branch.commit.hexsha[:8]}-{repo.active_branch.name}/ out/.config')
